...
@router.post("/materials", response_model=MaterialProfileRead, status_code=status.HTTP_201_CREATED)
def create_material(
...
@router.get("/materials/{material_id}", response_model=MaterialProfileRead)
...
@router.patch("/materials/{material_id}", response_model=MaterialProfileRead)
def update_material(
...
# ── Machine profiles ─────────────────────────────────────────────────
@router.get("/machines", response_model=list[MachineProfileRead])
...
@router.post("/machines", response_model=MachineProfileRead, status_code=status.HTTP_201_CREATED)
...
@router.get("/cost-assumptions", response_model=list[CostAssumptionSetRead])
...
@router.get("/cost-assumptions/{set_id}", response_model=CostAssumptionSetRead)
...
@router.patch("/cost-assumptions/{set_id}", response_model=CostAssumptionSetRead)
