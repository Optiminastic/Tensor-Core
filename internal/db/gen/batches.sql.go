...
const clearBatchSliceJob = `-- name: ClearBatchSliceJob :exec
UPDATE batches SET
...
func (q *Queries) ClearBatchSliceJob(ctx context.Context, id uuid.UUID) error {
	_, err := q.db.Exec(ctx, clearBatchSliceJob, id)
	return err
}

const countBedsInFlightPerFleetMachine = `-- name: CountBedsInFlightPerFleetMachine :many
...
type CountBedsInFlightPerFleetMachineRow struct {
...
func (q *Queries) CountBedsInFlightPerFleetMachine(ctx context.Context) ([]CountBedsInFlightPerFleetMachineRow, error) {
	rows, err := q.db.Query(ctx, countBedsInFlightPerFleetMachine)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []CountBedsInFlightPerFleetMachineRow{}
	for rows.Next() {
		var i CountBedsInFlightPerFleetMachineRow
		if err := rows.Scan(&i.FleetMachineID, &i.Beds); err != nil {
			return nil, err
		}
...
const releaseBatchAfterFailedPrint = `-- name: ReleaseBatchAfterFailedPrint :exec
UPDATE batches SET
    queue_item_id   = NULL,
    pipeline_run_id = NULL,
    print_outcome   = NULL,
...
const setBatchSliceJob = `-- name: SetBatchSliceJob :exec
UPDATE batches SET
    bambu_slice_job_id = $1,
...
type SetBatchSliceJobParams struct {
	BambuSliceJobID *int32
	FleetMachineID  *uuid.UUID
	ID              uuid.UUID
}
...
func (q *Queries) SetBatchSliceJob(ctx context.Context, arg SetBatchSliceJobParams) error {
	_, err := q.db.Exec(ctx, setBatchSliceJob, arg.BambuSliceJobID, arg.FleetMachineID, arg.ID)
	return err
}
