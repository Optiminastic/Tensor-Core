-- Projects. brand and status enums are read as text and written via text casts.

-- name: ListProjects :many
SELECT id, name, brand::text AS brand, description,
...
VALUES (sqlc.arg('id'), sqlc.arg('name'), sqlc.arg('brand')::brand,
        sqlc.narg('description'), sqlc.arg('status')::project_status, sqlc.arg('created_by'))
RETURNING id, name, brand::text AS brand, description,
          status::text AS status, created_by, created_at, updated_at;

-- name: UpdateProject :one
UPDATE projects SET
    name = COALESCE(sqlc.narg('name'), name),
    brand = COALESCE(sqlc.narg('brand')::brand, brand),
