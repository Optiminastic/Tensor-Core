// ============================================================================
//  DUAL-VIEW NAME PLATE  (DNP)  -  TWO-HEART VARIANT
//
//  Same plate as dual_name.scad, with two changes:
//     the two hearts come from centred padding  ->  heart NAME heart
//     MARGIN_L = MARGIN_R = 50 mm - equal gap on both sides, as specified
//
//  The names must differ in length for hearts to appear at all - the hearts
//  occupy the padding slots. Equal-length pairs produce no heart.
...
NAME_R      = "SUBHANTIKA";           // read from the RIGHT
PAD_L       = "center";           // "center" | "left" | "right"
PAD_R       = "center";
PAD_GLYPH   = "heart";            // fills a padding slot: "heart" | "none"
...
FILL        = 1.02;               // glyph width as a fraction of its cell.
                                  // Only sizes the optional PEDESTAL - it never
                                  // changed stroke weight, despite what the old
                                  // comment here claimed. Use STROKE for that.
STROKE      = 0;                  // 0 = the font exactly as drawn, which is what
                                  ...
                                  // every glyph outline, at the font's own
                                  // proportions (applied BEFORE the SQ squeeze),
                                  // so every stroke ends up 2*STROKE thicker.
                                  //   0  = the font exactly as drawn
                                  //   1  = a touch heavier
                                  //   2  = clearly bolder, still readable
                                  //   3+ = counters (the holes in A, O, R, S)
                                  //        pinch down to slits and N fills solid
                                  //   <0 = thinner; too negative and the light
                                  //        parts of a glyph break up
                                  // Those figures are for a full 9-slot run like
                                  // SANTOSH/SWATI; a short name is squeezed less
                                  // and takes more. The echo prints how many mm
                                  // a given STROKE actually adds.
...
// No extra slots are added. The two hearts come from the PADDING of the shorter
// name: PAD_* = "center" splits the spare slots evenly, so a 7/5 pair lands as
//     SANTOSH  /  heart SWATI heart
// which is exactly what the reference DNP.3mf contains.
...
// SQUEEZE is a manual override: it REPLACES the auto-fit, so nothing keeps the
// letters on the plate any more. Setting it above SQ_FIT stretches the glyphs
// instead of squeezing them and the run runs off the end of the bar.
if (SQUEEZE > 0)
    echo(str("*** SQUEEZE OVERRIDE ACTIVE: using ", SQUEEZE,
             " instead of the auto-fit ", min(SQ_FIT, SQ_WIDE),
             ". Set SQUEEZE = 0 to let the model fit itself to the plate."));
if (SQUEEZE > 0 && SQUEEZE > min(SQ_FIT, SQ_WIDE) * 1.001)
    echo(str("*** ERROR: SQUEEZE=", SQUEEZE, " is larger than the ",
             min(SQ_FIT, SQ_WIDE), " that fits. The letters WILL overrun the ",
             PLATE_L, " x ", PLATE_W, " plate. Set SQUEEZE = 0."));
...
if (SQ <= 0)
    echo("*** WARNING: STROKE is so large there is no width left for letters.");
...
// The thickness knob. STROKE == 0 short-circuits out of the glyph pipeline
// entirely, so a default plate still exports the model it always did.
//
...
module glyph2d(ch) {
    if (CONDENSE)
        ...
        // Fattening after the squeeze would add the same absolute mm to a
        // 0.17-scale upright as to a full-width crossbar, which slams the
        // counters (the holes in A, B, O, R) shut while the uprights are still
        // spindly. Doing it here keeps the letterform the font's shape.
