...
// resetSeedOrders clears a previous run's ORDERS and the jobs/batches they
// produced. Designs are deliberately kept: they are the product catalogue, they
// are idempotent by SKU, and deleting them would orphan anything else already
// referencing those SKUs.
//
// Order matters - production_jobs and batches are not FK'd to orders (order_id
// is ON DELETE SET NULL), so they are cleared explicitly before the rows they
// reference:
...
func resetSeedOrders(ctx context.Context, store *db.Store) error {
	stmts := []string{
...
// personalisationEveryN is how often a line item carries personalisation.
// Roughly one in five, so the Packaging/personalisation path is genuinely
// exercised without most of the queue being stuck awaiting confirmation.
const personalisationEveryN = 5

var personalisationNames = []string{"Ananya", "Vikram", "Priya", "Rahul", "Sneha", "Arjun", "Divya", "Karthik"}
var personalisationFonts = []string{"Inter", "Playfair", "Roboto Mono"}
...
func seedRealOrders(ctx context.Context, store *db.Store, designs []seededDesign, numRealOrders int) ([]uuid.UUID, error) {
	if len(designs) == 0 {
	...
	// Offset past whatever a previous additive run already created, so
	// shopify_order_id and the order number stay unique without a reset.
	base, err := nextSeedOrderOffset(ctx, store)
	if err != nil {
		return nil, err
	}
	...
	for n := 1; n <= numRealOrders; n++ {
		...
		// Every seeded order is paid. A "pending" order is a real Shopify state
		// but not one worth generating here: it adds a row the production flow
		// is not meant to act on, and the point of this data is to exercise the
		// order -> job -> batch -> machine path end to end.
		const financial = "paid"
		shopifyOrderID := int64(realOrderIDBase + base + n)

		order, err := store.Q.InsertOrder(ctx, gen.InsertOrderParams{
			ID: uuid.New(), ShopifyOrderID: shopifyOrderID,
			OrderNumber:  fmt.Sprintf("#TEST%04d", base+n),
...
// applyPersonalisation marks some line items as personalised.
//
// Most arrive complete, so AutoValidatePersonalisation resolves them to
// "validated" and they batch normally. A few deliberately arrive missing
// customer approval: those resolve to "pending", stay out of batching, and sit
// on the Packaging screen waiting for a human - which is the case that proves
// an unconfirmed job can never reach a plate.
func applyPersonalisation(rng *rand.Rand, li *production.LineItem) {
	if rng.Intn(personalisationEveryN) != 0 {
		return
	}
	name := personalisationNames[rng.Intn(len(personalisationNames))]
	font := personalisationFonts[rng.Intn(len(personalisationFonts))]
	li.PersonalisationRequired = true
	li.PersonalisationName = &name
	li.PersonalisationFont = &font
	li.CustomerApprovalRequired = true
	li.CustomerApprovalReceived = rng.Intn(4) != 0 // ~1 in 4 left unconfirmed
}

// nextSeedOrderOffset is how many seed orders already exist, so an additive run
// numbers its orders after them instead of colliding on shopify_order_id.
func nextSeedOrderOffset(ctx context.Context, store *db.Store) (int, error) {
	var n int
	row := store.Pool.QueryRow(ctx, "SELECT count(*) FROM orders WHERE source = 'seed'")
	if err := row.Scan(&n); err != nil {
		return 0, fmt.Errorf("count existing seed orders: %w", err)
	}
	return n, nil
}
